...
const WMO = {
  0: { d: "Sereno", e: "☀️" },
  1: { d: "Prevalentemente sereno", e: "🌤️" },
  2: { d: "Parzialmente nuvoloso", e: "⛅" },
  3: { d: "Coperto", e: "☁️" },
  45: { d: "Nebbia", e: "🌫️" },
  48: { d: "Nebbia con brina", e: "🌫️" },
  51: { d: "Pioviggine leggera", e: "🌦️" },
  53: { d: "Pioviggine moderata", e: "🌦️" },
  55: { d: "Pioviggine intensa", e: "🌧️" },
  56: { d: "Pioviggine gelata leggera", e: "🌧️" },
  57: { d: "Pioviggine gelata intensa", e: "🌧️" },
  61: { d: "Pioggia leggera", e: "🌦️" },
  63: { d: "Pioggia moderata", e: "🌧️" },
  65: { d: "Pioggia intensa", e: "🌧️" },
  66: { d: "Pioggia gelata leggera", e: "🌧️" },
  67: { d: "Pioggia gelata intensa", e: "🌧️" },
  71: { d: "Neve leggera", e: "🌨️" },
  73: { d: "Neve moderata", e: "🌨️" },
  75: { d: "Neve intensa", e: "❄️" },
  77: { d: "Granuli di neve", e: "🌨️" },
  80: { d: "Rovesci leggeri", e: "🌦️" },
  81: { d: "Rovesci moderati", e: "🌧️" },
  82: { d: "Rovesci violenti", e: "⛈️" },
  85: { d: "Rovesci di neve leggeri", e: "🌨️" },
  86: { d: "Rovesci di neve intensi", e: "❄️" },
  95: { d: "Temporale", e: "⛈️" },
  96: { d: "Temporale con grandine leggera", e: "⛈️" },
  99: { d: "Temporale con grandine forte", e: "⛈️" }
};
...
function buildMeteoEmbed(place, w) {
  ...
  return new EmbedBuilder()
    ...
    .setTitle(`${info.e} Meteo a ${place.name}`)
    .setURL(mapUrl)
...
async function handleMeteo(interaction) {
  ...
  try {
    ...
    if (!place) {
      await interaction.editReply({ embeds: [new EmbedBuilder().setColor(0xe53935).setTitle("❌ Località non trovata").setDescription(`Non ho trovato \`${posto}\`. Prova con un nome più preciso.`)] });
      return;
    }
    ...
    await interaction.editReply({ embeds: [new EmbedBuilder().setColor(0xe53935).setTitle("❌ Errore").setDescription("Impossibile recuperare il meteo: " + e.message)] });
  }
}
...
function buildForecastEmbed(place, w) {
  ...
  const e = new EmbedBuilder()
    .setColor(0x00897b)
    .setTitle(`📆 Previsioni a ${place.name} — prossimi ${d.time.length} giorni`)
    .setURL(mapUrl)
    .setDescription(placeName);
  for (let i = 0; i < d.time.length; i++) {
    const info = wmoInfo(d.weather_code[i]);
    const date = new Date(d.time[i]);
    const day = date.toLocaleDateString("it-IT", { weekday: "long", day: "numeric", month: "long", timeZone: "Europe/Rome" });
    const cap = day.charAt(0).toUpperCase() + day.slice(1);
    const value = `${info.e} **${info.d}**\n🌡️ ${d.temperature_2m_min[i].toFixed(0)}° / ${d.temperature_2m_max[i].toFixed(0)}°  •  🌧️ ${d.precipitation_sum[i]} mm (${d.precipitation_probability_max[i] ?? 0}%)  •  🌬️ ${d.wind_speed_10m_max[i].toFixed(0)} km/h`;
    e.addFields({ name: cap, value });
  }
  e.setFooter({ text: "Fonte: Open-Meteo" }).setTimestamp(new Date());
  return e;
}

async function handlePrevisioni(interaction) {
  const posto = interaction.options.getString("posto", true);
  await interaction.deferReply();
  try {
    const place = await geocode(posto);
    if (!place) {
      await interaction.editReply({ embeds: [new EmbedBuilder().setColor(0xe53935).setTitle("❌ Località non trovata").setDescription(`Non ho trovato \`${posto}\`.`)] });
      return;
    }
    const w = await getForecast(place.latitude, place.longitude, 5);
    await interaction.editReply({ embeds: [buildForecastEmbed(place, w)] });
  } catch (e) {
    console.error("/previsioni error:", e.message);
    await interaction.editReply({ embeds: [new EmbedBuilder().setColor(0xe53935).setTitle("❌ Errore").setDescription("Impossibile recuperare le previsioni: " + e.message)] });
  }
}

function alertColor(level) {
  const s = (level || "").toLowerCase();
  if (s.includes("rossa") || s.includes("elevata")) return { color: 0xd32f2f, emoji: "🔴", label: "ROSSA" };
  if (s.includes("arancione") || s.includes("moderata")) return { color: 0xf57c00, emoji: "🟠", label: "ARANCIONE" };
  if (s.includes("gialla") || s.includes("ordinaria")) return { color: 0xfbc02d, emoji: "🟡", label: "GIALLA" };
  return { color: 0x43a047, emoji: "🟢", label: "VERDE" };
}

function topAlertLevel(zone) {
  const fields = [zone["Per rischio idraulico"], zone["Per rischio temporali"], zone["Per rischio idrogeologico"], zone["Rappresentata nella mappa"]];
  const ranks = { "rosso": 4, "elevata": 4, "arancione": 3, "moderata": 3, "giallo": 2, "ordinaria": 2 };
  let max = { rank: 1, label: "VERDE" };
  for (const f of fields) {
    if (!f) continue;
    const s = f.toLowerCase();
    for (const k of Object.keys(ranks)) {
      if (s.includes(k) && ranks[k] > max.rank) {
        max = { rank: ranks[k], label: alertColor(s).label };
      }
    }
  }
  return max.label;
...
async function getCriticitaToday() {
  const j = await fetchJson(DATA_CRI);
  const node = j?.result?.data?.node;
  if (!node?.field_data_bollettino) throw new Error("Bollettino non disponibile");
  const id = node.field_data_bollettino;
  const topoUrl = `https://raw.githubusercontent.com/pcm-dpc/DPC-Bollettini-Criticita-Idrogeologica-Idraulica/master/files/topojson/${id}_today.json`;
  const topo = await fetchJson(topoUrl);
  const objKey = Object.keys(topo.objects)[0];
  const zones = topo.objects[objKey].geometries.map(g => g.properties);
  return { id, name: node.title, zones };
}

function searchZones(zones, query) {
  const q = query.toLowerCase().trim();
  return zones.filter(z => {
    const name = (z["Nome zona"] || "").toLowerCase();
    if (name.includes(q)) return true;
    const comuni = z["Comuni"] || [];
    return comuni.some(c => c.toLowerCase().includes(q));
  });
}

function buildAllertaEmbed(query, data, found) {
  if (!found.length) {
    return new EmbedBuilder()
      .setColor(0x9e9e9e)
      .setTitle(`❓ Nessuna zona trovata per "${query}"`)
      .setDescription("Prova con il nome di una **regione** italiana, una **città/comune**, o una **zona di allerta**.\nEsempi: `Lazio`, `Milano`, `Etna`, `Bacini Tordino`.")
      .setFooter({ text: "Fonte: Dipartimento della Protezione Civile" });
  }
  const sorted = found.slice().sort((a, b) => {
    const order = { "ROSSA": 0, "ARANCIONE": 1, "GIALLA": 2, "VERDE": 3 };
    return order[topAlertLevel(a)] - order[topAlertLevel(b)];
  });
  const top = topAlertLevel(sorted[0]);
  const c = alertColor(top);
  const e = new EmbedBuilder()
    .setColor(c.color)
    .setTitle(`${c.emoji} Allerta per "${query}" — Livello max: ${c.label}`)
    .setDescription(`📋 ${data.name}\n🔍 ${found.length} zona/e trovata/e (max 8 mostrate)`);
  for (const z of sorted.slice(0, 8)) {
    const lv = topAlertLevel(z);
    const ec = alertColor(lv);
    const lines = [
      `${ec.emoji} **${lv}**`,
      `💧 Idraulico: ${z["Per rischio idraulico"] || "—"}`,
      `⛈️ Temporali: ${z["Per rischio temporali"] || "—"}`,
      `⛰️ Idrogeologico: ${z["Per rischio idrogeologico"] || "—"}`
    ];
    e.addFields({ name: z["Nome zona"] || "Zona", value: clip(lines.join("\n"), 1000) });
  }
  if (found.length > 8) e.addFields({ name: "…", value: `Altre ${found.length - 8} zone non mostrate.` });
  e.addFields({ name: "🌐 Pagina ufficiale", value: `[Bollettino di Criticità](${PAGE_CRI})` });
  e.setFooter({ text: "Fonte: Dipartimento della Protezione Civile" }).setTimestamp(new Date());
  return e;
}

async function handleAllerta(interaction) {
  const posto = interaction.options.getString("posto", true);
  await interaction.deferReply();
  try {
    const data = await getCriticitaToday();
    const found = searchZones(data.zones, posto);
    await interaction.editReply({ embeds: [buildAllertaEmbed(posto, data, found)] });
  } catch (e) {
    console.error("/allerta error:", e.message);
    await interaction.editReply({ embeds: [new EmbedBuilder().setColor(0xe53935).setTitle("❌ Errore").setDescription("Impossibile recuperare le allerte: " + e.message)] });
  }
}
...
async function registerCommands(client) {
  const commands = [
    new SlashCommandBuilder()
      .setName("meteo")
      .setDescription("Meteo attuale per una località")
      .addStringOption(o => o.setName("posto").setDescription("Nome della località (es. Roma, Milano)").setRequired(true))
      .toJSON(),
    new SlashCommandBuilder()
      .setName("previsioni")
      .setDescription("Previsioni dei prossimi 5 giorni per una località")
      .addStringOption(o => o.setName("posto").setDescription("Nome della località (es. Roma, Milano)").setRequired(true))
      .toJSON(),
    new SlashCommandBuilder()
      .setName("allerta")
      .setDescription("Livello di allerta meteo-idro per una zona, città o regione italiana")
      .addStringOption(o => o.setName("posto").setDescription("Regione, città o zona (es. Lazio, Milano, Etna)").setRequired(true))
      .toJSON()
  ];
  ...
  try {
    if (guildId) {
      ...
      console.log("Slash commands registrati sul server " + guildId);
    } else {
      await rest.put(Routes.applicationCommands(client.user.id), { body: commands });
      console.log("Slash commands registrati globalmente (può richiedere fino a 1 ora)");
    }
  } catch (e) {
    console.error("Errore registrazione comandi:", e.message);
  }
}
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.commandName === "meteo") await handleMeteo(interaction);
  else if (interaction.commandName === "previsioni") await handlePrevisioni(interaction);
  else if (interaction.commandName === "allerta") await handleAllerta(interaction);
});
